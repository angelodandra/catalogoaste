...
async function load(silent: boolean = false) {
   ...
   *
   * REGOLA CHIAVE: tutto si calcola sul **peso interno** (peso reale del
   * pesce comprato all'asta) e sul **prezzo netto €/Kg**. Il peso
   * pubblicato (peso_interno + ghiaccio) è solo dato di presentazione e
   * NON entra nel calcolo costo/ricavo/ricarico.
   *
   * Costo €/Kg = cost_eur (totale lotto) / peso_interno_kg
   *   = prezzo asta €/Kg + (spese ripartite per Kg di pesce)
   * Vendita €/Kg = prezzo netto inserito (così com'è).
   * Ricavo previsto = vendita €/Kg × peso interno (il pesce reale che vendi).
   * Margine = ricavo - costo
   * Ricarico % = margine / costo × 100
   *
   * Il valore in fattura sarà poi calcolato alla consegna sul peso
   * effettivo pesato in quel momento × prezzo €/Kg.
   */
  function computeRow(
    priceStr: string,
    _weightStr: string, // peso pub. kg — non usato per il calcolo ricarico
    costTot: number | null,
    pesoInternoKg: number | null
  ) {
    const validInterno = pesoInternoKg != null && pesoInternoKg > 0;

    // Costo €/Kg sul peso INTERNO
    const costPerKg =
      costTot != null && costTot > 0 && validInterno
    ...
    // pricePerKg = quello inserito (è già €/Kg netto)
    const pricePerKg = validPrice ? Math.round(price! * 100) / 100 : null;

    // Ricavo previsto = vendita €/Kg × peso interno (pesce reale)
    const revenueTot =
      validPrice && validInterno
